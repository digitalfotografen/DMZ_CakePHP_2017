    ...
    'Users.SimpleRbac.permissions' => [
        ...
        [
            ...
            'allowed' => function (array $user, $role, \Cake\Network\Request $request) {
                $visitId = \Cake\Utility\Hash::get($request->params, 'pass.0');
                $visit = \Cake\ORM\TableRegistry::get('Visits')->get($visitId);
                $userId = $user['id'];
                if (!empty($visit->user_id) && !empty($userId)) {
                    return $visit->user_id === $userId;
                }
                return false;
            }
        ],
